...
#version of alt_fetch that assigns very high random energy and power values to each appliance
#good for checking errors that sometimes arise when values get high
...
time_id=0
last_id=0
appliance_data = ['microwave', 'fridge', 'oven', 'rangehood', 'stove']
apps = []
CSV_FILE = '../../data/ApplianceData4.csv'
...
def writeCSV():
    with open(CSV_FILE, 'wb') as csvfile:
        ...
        if(apps[num_app].energy == 0.0):
            apps[num_app].energy = float(random.randint(-2000.0,1000.0))
            apps[num_app].power = float(random.randint(0.0,500.0))
        else:
