//defines rate at which hexagons grow -- smaller denominator = faster growth
function energy_to_size(energy) { return STARTING_SIZE + energy/3  }
...
function Hive(path, positions, size){
    ...
    this.draw = function() {
      ...
      //if the loop would run past the number of possible coordinates,
      //only run it as long as the list of coordinates
      if (this.positions.length*power_to_density(this.appliance) > this.positions.length) {
        for (var i = 0; i < this.positions.length; i++) {
      	  ctx.drawImage(this.img, this.positions[i].x - this.size/2, this.positions[i].y - this.size/2, this.size, this.size);
        }
      //otherwise, run it the proper amount
      } else {
