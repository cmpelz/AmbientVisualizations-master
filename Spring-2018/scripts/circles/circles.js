//defines rate at which circles grow -- smaller denominator = faster growth
function energy_to_size(energy) { return STARTING_SIZE + energy/5 }
//defines rate at which circles become opaque -- smaller denominator = faster transition to opactity
function power_to_opacity(appliance) { return appliance.get_power()/(appliance.get_max_power()/2); }
...
function ColoredCircle(path,x,y,diameter,page_height,page_width){
    ...
    /* assign an appliance to the circle */
    this.addAppliance = function(appliance){
        this.appliance = appliance;
    },
    /* change opacity of the circle based off of the appliance's power */
    this.draw = function() {
    ...
    this.update = function() {
      ...
      if(this.appliance.running){
    	...
    	/* keep the circle in the same position the entire time */
    	this.grow()
    	this.move();
